<p align="center">
  <img src="https://img.shields.io/badge/version-2.5-blue?style=flat-square" alt="Version 2.5">
...
**V2.4 本地修复版**：V2.4 聚焦疫苗登记增强、消息红点与搜索增强、搜索页视觉升级、设置项有效性增强、欢迎页与主导航体验优化，以及后台管理体验优化。

**V2.5 移动体验升级版**：V2.5 聚焦喵喵台圆盘上传回归、登录注册错误提示、普通用户修改密码、AI 养育异常预警、广场动态卡片、村长推荐与后台热门话题管理。

**V2.5.1 小补丁**：优化喵喵台音频分析 loading、长超时与历史刷新轮询；继续打磨登录注册和 Settings 页面；发布动态支持从历史情绪导入，广场与详情页展示猫咪情绪和叫声。

**V2.5.2 极小补丁**：整理发布动态“添加猫语”为上传录音与历史导入两个入口；上传前校验音频时长与大小；继续统一登录注册主按钮视觉。
...
- **喵喵台圆盘上传回归**：放弃实时录音策略，恢复点击圆盘选择音频文件并调用原有猫叫分析接口。
- **登录/注册体验增强**：移动端布局更紧凑，错误提示透传后端友好信息，避免显示技术栈文本。
- **普通用户修改密码**：设置页新增账户安全入口，验证原密码并修改成功后重新登录。
- **AI 养育助手增强**：快捷入口调整，异常预警调用 AI 基于猫咪档案、疫苗和喵喵台记录输出重点建议，疫苗 chip 防溢出。
- **广场动态卡片优化**：列表正文限制 3-4 行，展开进入详情；村长推荐徽章更醒目；位置展示真实字段或优雅降级。
- **热门话题管理**：后台支持新增、编辑、删除、展示开关、推荐开关和排序，普通端热门话题按后台排序优先展示。

## ✨ V2.4 核心特性

### 💉 疫苗登记增强
- 添加/编辑猫咪档案支持预设疫苗与多个自定义疫苗
- 自定义疫苗会去空、去重并保存到现有 `vaccine_status` 字符串字段
- 编辑页可回显旧格式疫苗字符串，档案页和喵喵卡继续展示同一份疫苗信息

### 💬 消息红点与搜索增强
- 消息中心新增私信、评论、赞、粉丝四类红点/数量
- 底部消息 Tab 显示四类通知总数，超过 99 显示 `99+`
- 会话搜索接入后端接口，支持昵称、用户名、最近消息和历史消息内容匹配

### 🔎 搜索与设置体验
- `/social/search` 页面视觉升级，热门话题复用真实接口，动态搜索接入后端
- 通知设置影响红点展示，隐私设置影响本机资料可见性与互动入口
- Welcome 页面更贴合猫咪社交、喵喵情绪识别、AI 养育助手和健康档案定位

### 🛠 管理与导航优化
- 底部“养护”改为“AI养育”，并优化消息红点展示
- AI 养育助手无猫咪时提示更清楚，隐藏聊天输入和快捷入口
- 后台管理保留 3×3 统计卡，同时优化运营状态、Tab 与移动端按钮布局
...
| 版本 | 日期 | 核心变更 |
|:---|:---|:---|
| **V2.5.2** | 2026-07 | 极小补丁：发布动态添加猫语入口精简为上传录音/历史导入；喵喵台与发布动态上传前限制音频大小和 5-20 秒建议时长；登录注册主按钮更贴近 Welcome 风格 |
| **V2.5.1** | 2026-07 | 小补丁：喵喵台音频分析阶段提示、长超时和历史刷新轮询；登录注册与 Settings 视觉优化；发布动态支持历史情绪导入；广场列表和动态详情展示猫咪情绪快照与音频 |
| **V2.5** | 2026-07 | 移动体验升级版：喵喵台恢复圆盘上传音频并取消实时录音；登录/注册错误提示与紧凑布局优化；设置页新增普通用户修改密码；AI 养育异常预警与疫苗展示防溢出；广场动态截断、展开详情、村长推荐样式和真实位置降级；后台热门话题新增管理与排序能力 |
| **V2.4** | 2026-07 | 💡 本地修复版：疫苗登记支持预设与多个自定义疫苗；消息红点汇总私信/评论/赞/粉丝并接入底部 Tab；会话搜索与社区搜索接入后端；搜索页、欢迎页、AI 无猫咪状态、猫咪档案添加文案、设置项有效性和后台管理移动端体验优化 |
