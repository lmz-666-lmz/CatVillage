...
type BackendStatisticsResponse = {
  ...
  mood_trend?: Array<{ date: string; label?: string; emotion_tag?: string; confidence: number | null }>;
...
type BackendWeeklyReportResponse = {
  ...
  total: number;
  distribution: Array<{ label: string; count: number; percentage: number }>;
  mood_trends?: Array<{ date: string; label?: string; emotion_tag?: string; confidence: number | null }>;
  avg_confidence?: number;
  summary: string;
  week_start?: string;
  week_end?: string;
...
const toMoodTrend = (item: { date: string; label?: string; emotion_tag?: string; confidence: number | null }) => ({
  date: item.date,
  emotionTag: item.emotion_tag || item.label || '',
  confidence: item.confidence ?? 0
});

export async function createAuthorizedAudioObjectUrl(audioUrl: string): Promise<string> {
...
export function recognizeEmotion(data: FormData): Promise<ApiResponse<RecognizeEmotionResponse>> {
  ...
  if (!normalizedPetId || normalizedPetId === 'undefined') {
    return Promise.reject(new Error('请先添加猫咪'));
  }
  normalized.append('pet_id', normalizedPetId);
  if (audioFile) {
...
export function deleteEmotionRecord(recordId: string): Promise<ApiResponse<{ success: boolean }>> {
...
export function getEmotionStatistics(params: { catId?: string; startDate?: string; endDate?: string }): Promise<ApiResponse<EmotionStatisticsResponse>> {
  return request<BackendStatisticsResponse>({
    ...
    ...response,
    data: {
      moodTrend: (response.data.mood_trend || []).map(toMoodTrend),
...
export function getWeeklyReport(params: { catId?: string; week?: string }): Promise<ApiResponse<WeeklyReportResponse>> {
  return request<BackendWeeklyReportResponse>({
    ...
    data: {
      ...
      moodTrends: (response.data.mood_trends || []).map(toMoodTrend),
