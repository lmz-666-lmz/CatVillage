<template>
  <div class="min-h-[100dvh] bg-[#f4f5fb] px-4 pt-4 pb-8">
    <header class="flex items-center gap-2">
      ...
      <h1 class="text-[20px] font-black text-[#12182a]">隐私设置</h1>
    </header>
    ...
    <section class="mt-4 space-y-3 rounded-[18px] bg-white p-3 shadow-[0_4px_14px_rgba(20,27,43,0.05)]">
      <van-cell title="允许被搜索到" label="他人可通过喵村号搜索到你" center>
        <template #right-icon>
          <van-switch v-model="config.searchable" active-color="#b33500" />
        </template>
      </van-cell>
      <van-cell title="公开动态互动" label="允许陌生人评论和点赞" center>
        <template #right-icon>
          <van-switch v-model="config.openInteraction" active-color="#b33500" />
        </template>
      </van-cell>
    </section>
  </div>
