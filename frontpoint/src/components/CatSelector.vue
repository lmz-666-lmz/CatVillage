<template>
  <div class="cat-selector">
    ...
    <div v-show="isOpen" class="selector-content">
      ...
      >
        <img
          :src="getSafeCatAvatarUrl(cat.avatarUrl)"
          :alt="cat.name"
          class="cat-avatar"
      ...
      <div v-if="cats.length === 0" class="no-cats">
        <span class="no-cats-emoji">🐱</span>
        <p class="no-cats-title">还没有猫咪档案</p>
        <p class="no-cats-desc">添加一只猫咪后再来选择</p>
        <van-button class="no-cats-btn" size="small" type="primary" @click="$router.push({ name: 'AddCat' })">
          <van-icon name="plus" size="14" /> 添加猫咪
        </van-button>
      </div>
    </div>
...
import { getSafeCatAvatarUrl, handleImageError } from '@/utils/image';
...
.toggle-btn:active { background: #f5f5f3; }
...
.no-cats {
  ...
  border-radius: 16px;
  background: linear-gradient(135deg, #fff8f4, #fff, #fef5ff);
}
.no-cats-emoji { font-size: 48px; display: block; margin-bottom: 8px; }
.no-cats-title { font-size: 16px; font-weight: 700; color: #1f2937; margin: 0 0 4px; }
.no-cats-desc { font-size: 13px; color: #9CA3AF; margin: 0 0 16px; }
.no-cats-btn { --van-button-primary-background: #ff6b35; --van-button-primary-border-color: #ff6b35; }
