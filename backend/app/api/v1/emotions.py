...
def _build_mood_trend(records: list[EmotionRecord]) -> list[dict]:
    sorted_records = sorted(records, key=lambda item: (item.record_time or datetime.min).isoformat())
    return [
        {
            "date": record.record_time.date().isoformat() if record.record_time else "",
            "label": record.label,
            "emotion_tag": record.label,
...
def _calculate_avg_confidence(records: list[EmotionRecord]) -> float:
    confidences = [record.confidence for record in records if record.confidence is not None]
    return round(sum(confidences) / len(confidences), 4) if confidences else 0
...
async def recognize_cat_emotion(
    ...
    # 验证音频格式
    if not file.filename.lower().endswith((".wav", ".mp3", ".mpeg", ".m4a", ".aac", ".flac", ".webm", ".mp4", ".ogg")):
...
def get_emotion_statistics(
    ...
    mood_trend = _build_mood_trend(records)
    avg_confidence = _calculate_avg_confidence(records)
...
def get_weekly_report(
    ...
    mood_trends = _build_mood_trend(records)
    avg_confidence = _calculate_avg_confidence(records)
